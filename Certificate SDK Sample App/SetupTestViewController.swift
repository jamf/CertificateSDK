...
class SetupTestViewController: UITableViewController {
    ...
    /// This variable holds the struct that determines what kind of test will occur when
    // the "Run Test" button is pressed.
    var setup = CertificateSDKSetup(isActual: false, slowSpeed: true, simulateError: false)
    ...
    @IBAction func selectType(_ sender: UISegmentedControl) {
        ...
        if newActual != setup.isActual {
            setup.isActual = newActual
    ...
    /// The user has turned on/off an option within the setup.
    ///
    ...
    @IBAction func toggleOption(_ sender: UISwitch) {
        ...
        switch index {
        case .optionSpeed:
            setup.slowSpeed = sender.isOn

        case .optionError:
            setup.simulateError = sender.isOn
    ...
    @IBAction func scheduleNotification(_ sender: UIButton) {
        let notificationHandler = UIApplication.shared.delegate as? LocalNotificationHandler
        // A real app should schedule the notification to renew on some date in the future.
        notificationHandler?.scheduleLocalNotificationForRenewal(Date(timeIntervalSinceNow: 60))
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return (setup.isActual ? SetupNumberOfRows.actualTest : SetupNumberOfRows.simulatedTest).rawValue
    }
    ...
    private func cell(for index: SetupRowIndices, at indexPath: IndexPath) -> UITableViewCell {
        ...
        case .typeOfTest:
            ...
            cell?.segmentedControl.selectedSegmentIndex = (setup.isActual ? 1 : 0)
            return cell!
        case .descriptionOfTestType:
        ...
        case .optionSpeed:
            ...
            cell?.optionSwitch.isOn = setup.slowSpeed
            return cell!
        case .optionError:
            let cell = tableView.dequeueReusableCell(withIdentifier: "Option", for: indexPath) as? OptionCellView
            ...
            cell?.optionSwitch.isOn = setup.simulateError
            return cell!
        case .runTestButton:
